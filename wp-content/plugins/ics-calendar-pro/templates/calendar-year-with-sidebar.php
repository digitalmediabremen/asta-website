...
<section class="<?php echo esc_attr(implode(' ', $ics_calendar_classes)); ?>" id="<?php echo esc_attr($ics_data['guid']); ?>">
	...
	// Display calendar
		...
		<div class="ics-calendar-main-with-sidebar ics-calendar-year-with-sidebar">

			<div class="ics-calendar-main">
		
				<?php
				if (count($ics_data['events']) > 1) {
					?>
					<select class="ics-calendar-select" autocomplete="off" style="display: none;">
						<?php
						// Build list from earliest to latest year
						foreach (array_keys((array)$ics_data['events']) as $year) {
							?>
							<option value="<?php echo esc_attr($year); ?>"<?php if ($year == date('Y')) { echo ' selected="selected"'; } ?>><?php echo $year; ?></option>
							<?php
						}
						?>
					</select>
				...
				// Build yearly/monthly calendars
				foreach (array_keys((array)$ics_data['events']) as $year) {
					?>
		
					<div class="ics-calendar-year-wrapper" data-year="<?php echo esc_attr($year); ?>"<?php if (count($ics_data['events']) > 1) { echo ' style="display: none;"'; } ?>>
						...
						for ($m = 1; $m <= 12; $m++) {
							...
							$month_label = ucwords(wp_date('F', gmmktime(0,0,0,$month,1,$year), $R34ICS->tz));
			
							// Build month's calendar
							?>
							<div class="ics-calendar-year-month" data-year="<?php echo esc_attr($year); ?>">
					
								<h5 class="ics-calendar-label">
									<?php echo $month_label; ?>
								</h5>
						
								<table class="ics-calendar-year-month-grid ics-calendar-widget-grid">
									<thead>
										<tr>
											<?php
