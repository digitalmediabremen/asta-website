...
class R34ICS {
	...
	public function color_key_html($args, $ics_data) {
		...
		if (count((array)$ics_data['colors']) > 1 && count((array)$ics_data['feed_titles']) > 1) {
			?>
			<div class="is-style-centered-mobile ics-calendar-color-key<?php if (!empty($args['legendinline'])) { echo ' inline'; } ?>">
				<?php
				if (count($ics_data['feed_titles']) > 4) {
					$toggle_all_guid = r34ics_guid();
					?>
					<div class="ics-calendar-color-key-header">
                        <input type="checkbox" id="<?php echo esc_attr($toggle_all_guid); ?>" class="ics-calendar-color-key-toggle-all" data-feed-key="ALL" checked="checked" />
						<label for="<?php echo esc_attr($toggle_all_guid); ?>">
				...
				foreach ((array)$ics_data['feed_titles'] as $feed_key => $feed_title) {
					...
					<div class="ics-calendar-color-key-item" data-feed-key="<?php echo intval($feed_key); ?>">
                        <input type="checkbox" id="<?php echo esc_attr($toggle_guid); ?>" class="ics-calendar-color-key-toggle" data-feed-key="<?php echo intval($feed_key); ?>" checked="checked" />
						<label for="<?php echo esc_attr($toggle_guid); ?>">
						<?php
						echo $feed_title;
				...
				if ($ICal->hasEvents() && $ics_events = $ICal->eventsFromRange($range_start,$range_end)) {
					...
					foreach ((array)$ics_events as $event) {
						...
						$event_item = array(
							...
							'feed_key' => $feed_key,
                            'calendar_title' => $ICal->calendarName(),
							'calendar_description' => $ICal->calendarDescription(),
                            'feed_title' => $ics_data['feed_titles'][$feed_key],
						);
