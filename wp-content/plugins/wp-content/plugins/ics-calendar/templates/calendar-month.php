	...
	// Display calendar
		...
		foreach (array_keys((array)$ics_data['events']) as $year) {
			for ($m = 1; $m <= 12; $m++) {
				...
				?>
				<article class="ics-calendar-month-wrapper" data-year-month="<?php echo date('Ym', gmmktime(0,0,0,$month,1,$year)); ?>">
											...
											foreach ((array)$day_events as $time => $events) {
												...
												foreach ((array)$events as $event) {
													...
													if ($time == 'all-day') {
														...
														?>
														<li class="<?php echo r34ics_event_css_classes($event, $time, $args); ?>" data-feed-key="<?php echo intval($event['feed_key']); ?>">
															<span class="calendar-name"><?php echo $event['feed_title']; ?></span>
                                                            <?php
